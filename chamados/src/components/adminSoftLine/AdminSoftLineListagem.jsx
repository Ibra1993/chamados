...
export default function AdminSoftLineListagem({ vetor = [] }) {
 ...
 if (!colaborador.trim()) {
   ...
   try {
     const response = await fetch(`http://localhost:8080/chamados/user/listarPorColaborador?nome=${colaborador}`);
...
useEffect(() => {
    ...
    const fetchChamados = async () => {
        try {
            const response = await axios.get("http://localhost:8080/chamados/user/userListChamados", {
                headers: {
